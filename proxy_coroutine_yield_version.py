...
class WebSocketProxpy:
    ...
    def proxy_dispatcher(self, proxy_web_socket, path):
        ...
        if not self.is_forced_url_no_password_server():
            ...
            if self.authenticate(connection):
                ...
                #await proxy_web_socket.send(get_json_status_response("ok", authenticated_message + "'}"))
                ...
                await self.process_arbitrary_requests(proxy_web_socket, proxied_web_socket, connection,proxied_url_value)
            else:
            ...
            for i in range(len(self.proxied_port_list)):
                ...
                if i == len(self.proxied_port_list) - 1:
                    try:
                        await proxy_web_socket.send('{"receive_error": "PROXIED SERVER\'s whole ports are using NOW!"}')
                        ...
                        break
                    await proxy_web_socket.wait_closed()
    ...
    def process_arbitrary_requests(self, proxy_web_socket, proxied_web_socket, connection):
        ...
        while not proxied_web_socket.closed:
            ...
            try:
                request_for_proxy = await proxy_web_socket.recv()
                # request_for_proxy = await asyncio.wait_for(proxy_web_socket.recv(), timeout=10)
            except websockets.exceptions.ConnectionClosed:
                 ...
                 await proxied_web_socket.close(code=1000)
            ...
            if self.is_close(request_for_proxy):
                ...
                # 关闭proxy_web_socket
                await proxy_web_socket.send('{"action": "closed"}')
                await proxy_web_socket.wait_closed()
            ...
            try:
                ...
                #关闭proxied_web_socket
                await proxied_web_socket.wait_closed()
                self.logger.warning("PROXIED SERVER [ " + proxied_web_socket.host + ":" + str(proxied_web_socket.port) + " ] is closed")
                #关闭proxy_web_socket

                await proxy_web_socket.close()
                self.logger.warning("PROXY SERVER [ " + proxy_web_socket.remote_address[0] + ":" + str(proxy_web_socket.remote_address[1]) + " ] is closed")

                is_port_used[str(proxied_web_socket.port)] = False
                return
                # proxied_web_socket = await websockets.connect(proxied_url_value,ping_interval=20,ping_timeout=20, close_timeout=10)
                # await self.send_to_web_socket_connection_aware(proxy_web_socket, proxied_web_socket,
                #                                                     request_for_proxy)
                # response_from_proxy = await proxied_web_socket.recv()
